...
def sort_tokens(args, token_query, token_row_data):
    ...
    # elif args.tokens_sort_method == "SUM":
    #     fasttext_model = fasttext.FastText.load_model(args.fasttext_model)
    #     idx_cosine = []
    #     for token in token_row_data:
    #         token_vec = fasttext_model.get_word_vector(token)
    #         cos_score = 0
    #         for q_token in token_query:
    #             q_token_vec = fasttext_model.get_word_vector(q_token)
    #             cos_score += dis.cosine(token_vec,q_token_vec)
    #         idx_cosine.append(cos_score)
    #     idx_cosine = np.array(idx_cosine)
    #     indexs = np.argsort(idx_cosine)  # 按照与query的相似度对token进行排序返回的索引
    #     return [token_row_data[each] for each in indexs]
    #
    # elif args.tokens_sort_method == "AVG":
    #     fasttext_model = fasttext.FastText.load_model(args.fasttext_model)
    #     query_vec = np.mean([fasttext_model.get_word_vector(token) for token in token_query], axis=0)
    #     idx_cosine = []
    #     for token in token_row_data:
    #         token_vec = fasttext_model.get_word_vector(token)
    #         idx_cosine.append(dis.cosine(token_vec,query_vec))
    #     idx_cosine = np.array(idx_cosine)
    #     indexs = np.argsort(idx_cosine)  # 按照与query的相似度对token进行排序返回的索引
    #     return [token_row_data[each] for each in indexs]
    #
    # elif args.tokens_sort_method == "MAX":
    #     fasttext_model = fasttext.FastText.load_model(args.fasttext_model)
    #     idx_cosine = []
    #     for token in token_row_data:
    #         token_vec = fasttext_model.get_word_vector(token)
    ...
    #         for q_token in token_query:
    #             q_token_vec = fasttext_model.get_word_vector(q_token)
    #             best_score = min(dis.cosine(token_vec,q_token_vec),best_score)
    #         idx_cosine.append(best_score)
    #     idx_cosine = np.array(idx_cosine)
    #     indexs = np.argsort(idx_cosine)  # 按照与query的相似度对token进行排序返回的索引
    #     return [token_row_data[each] for each in indexs]
...
def convert_examples_to_features(args, examples, tokenizer):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        _truncate_seq_pair(args, token_query, token_row_data,
        ...
        if args.add_caption and example.caption:
            ...
            _truncate_seq_pair(args, token_caption, token_row_data,
                               max_seq_length - len(tokens) - 2)
            tokens += token_caption + ['SEP']
            segment_ids += [0] * (len(token_caption) + 1)
        ...
        if args.add_headings and example.headings:
            ...
            _truncate_seq_pair(args, token_headings, token_row_data,
                               max_seq_length - len(tokens) - 2)
            tokens += token_headings + ['SEP']
            segment_ids += [0] * (len(token_headings) + 1)
...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        if eval_loss < best_loss:
            logger.info("Get new best loss %s", eval_loss)
            best_loss = eval_loss
...
def main():
    ...
    if args.use_wtr:
        args.exp_name = '0_fold_wtr'
    else:
        args.exp_name = '0_fold'
    if args.add_caption:
        args.exp_name += "_add_caption"
    if args.add_headings:
        args.exp_name += "_add_headings"
    ...
    if args.use_wtr:
        args.train_dir = "../data/wdc_0_train.jsonl"
    else:
        args.train_dir = '../data/0_train.jsonl'
    if args.use_wtr:
        args.test_dir = "../data/wdc_0_test.jsonl"
    else:
        args.test_dir = '../data/0_test.jsonl'
